...
cria_copia_unidade(unidade) recebe uma unidade u e devolve uma nova copia da unidade.
    """      
    return cria_unidade(cria_copia_posicao(unidade["pos"]),unidade["vida"],unidade["forca"],unidade["exercito"])  
...
cria_copia_mapa(mapa) recebe um mapa e devolve uma nova copia do mapa 
    ...
    return cria_mapa(mapa["dim"],Wall_new,e1_new,e2_new)
    ...
    for y in range(obter_pos_y(dim)): #Em cada linha 
        for x in range(obter_pos_x(dim)): #Em cada coluna 
            if eh_posicao_parede(mapa,cria_posicao(x,y)): #Ver se e parede 
                stri = stri + "#"
            elif eh_posicao_unidade(mapa,cria_posicao(x,y)): #Se e posicao 
...
adjacentes a unidade u de acordo com a ordem de leitura do labirinto.
    """            
    res = ()
    for nome in obter_nome_exercitos(mapa):
        if obter_exercito(unidade) != nome:
            inimigo = nome
    adjacentes = obter_posicoes_adjacentes(obter_posicao(unidade))
    for adj in adjacentes:
        uni = obter_unidade(mapa,adj)
        if uni != None  and obter_exercito(uni) == inimigo:
            res = res + (uni,)
    return res
...
def simula_batalha(string,T_F):
    ...
    f1.close()
    for pos in pose1:
        e1 = (cria_unidade(pos,e1_misc[1],e1_misc[2],e1_misc[0]),) + e1
    for pos in pose2:
        e2 = (cria_unidade(pos,e2_misc[1],e2_misc[2],e2_misc[0]),) + e2
    mapa = cria_mapa(dim,wall,e1,e2)
    
    Empate = False
    Maior = False 
    if e1_misc[0] < e2_misc[0]:
        Maior = True
    se_true =("[ " + str(e1_misc[0]) + ":" + str(calcula_pontos(mapa,e1_misc[0])) + " " + str(e2_misc[0]) + ":" + str(calcula_pontos(mapa,e2_misc[0])) +" ]")
    se_false = ("[ " + str(e2_misc[0]) + ":" + str(calcula_pontos(mapa,e2_misc[0])) + " " + str(e1_misc[0]) + ":" + str(calcula_pontos(mapa,e1_misc[0])) +" ]")
    ...
    while calcula_pontos(mapa,e1_misc[0]) != 0 and calcula_pontos(mapa,e2_misc[0]) != 0:
    ...
    if Empate == True:
        ...
        if calcula_pontos(mapa,e1_misc[0]) == 0:
            return (e2_misc[0])
        else:
            return (e1_misc[0])